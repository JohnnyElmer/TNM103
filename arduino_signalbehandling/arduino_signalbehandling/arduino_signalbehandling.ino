...
int min_;
int max_;
byte sramBuffer[1024]; // SRAM buffer, "ljudminne", 8-bitar
...
void loop()
{
  ...
  //pseudo-code for overdrive
  soundSampleFromADC = badc0;
  /*temp = ((soundSampleFromADC - 127)*badc1)/4; //???????????

  if(temp > max_) temp = max_;
  if(temp < min_) temp = min_;

  
  temp = temp + 127;
  sramBufferSampleValue = temp;*/

  /*
  sramBuffer[bufferIndex] = soundSampleFromADC;
  soundSampleFromSramBuffer = sramBuffer[bufferIndex2];
  bufferIndex++;
  bufferIndex2 = bufferIndex + (badc1*2);

  bufferIndex = bufferIndex%512;
  bufferIndex2 = bufferIndex2&511;

  sramBufferSampleValue=((sramBuffer[bufferIndex]-127)+(soundSampleFromSramBuffer-127)+127)/2;
  */

  sramBufferSampleValue = sramBuffer[bufferIndex];
  
  soundSampleFromSramBuffer = ((127-sramBufferSampleValue)*(badc1/255.0)) + (soundSampleFromADC-127);

  max_ = 127;
  min_ = -127;
  if(soundSampleFromSramBuffer > max_) soundSampleFromSramBuffer = max_;
  if(soundSampleFromSramBuffer < min_) soundSampleFromSramBuffer = min_;

  sramBufferSampleValue = soundSampleFromSramBuffer + 127;
  sramBuffer[bufferIndex] = sramBufferSampleValue;
  bufferIndex++;
  bufferIndex = bufferIndex%1024;
  
  OCR2A =  sramBufferSampleValue;//sramBufferSampleValue;
