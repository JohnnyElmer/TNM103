...
void loop()
{
  ...
  if(badc1 == 0) bufferIndex += 1;
  else bufferIndex += 200;
  bufferIndex2 = bufferIndex*3;
  bufferIndex = bufferIndex%512;
  bufferIndex2 = bufferIndex2%512;
  
  sramBufferSampleValue = sramBuffer[bufferIndex];
  //sramBufferSampleValue = (sramBuffer[bufferIndex]+sramBuffer[bufferIndex2])/2;
...
/*******************************************/
  ...
  //lowpassLower
  //soundSampleFromADC = badc0; //låt från dator
  soundSampleFromADC = sramBufferSampleValue;
  alpha = 0.1*map(badc1,0,255,0,10);
  filteredSample=((soundSampleFromADC*alpha) + (previousSample*(1-alpha)));
  previousSample=filteredSample;

  //bandpass
  alpha2 = sqrt(alpha);
  lowpassUpper=((soundSampleFromADC*alpha2) + (previousSample2*(1-alpha2)));
  bandpass= (lowpassUpper-127) - (filteredSample-127)+127;
  previousSample2=lowpassUpper;

  //bandstopp;
  bandstop = ((soundSampleFromADC-127) - (bandpass-127))+127;

  //högpass
  highpassFiltered = (soundSampleFromADC-127) - (filteredSample-127)+127;

  
  OCR2A = filteredSample;
...
float soundValue = 0;
  ...
  for(int i = 0; i < sizeof(sramBuffer); i++){
    ...
    */
    ...
    /*
    //Fyrkant + mixad (sista sramBuffer)
    if (i < 255) soundValue1 = 192;
    else soundValue1 = 64;
    sramBuffer[i] = soundValue1;
    //sramBuffer[i] = ((soundValue1+sinusSample)/2)-127 ;
    */
